...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        ...
        $stmt = $pdo->prepare("
            ...
            WHERE id = ?
        ");
        $stmt->execute([$name, $category_id, $brand, $price, $sku, $availability, $description, $specs_json, $features_json, $product_id]);
...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .image-item .delete-image { position: absolute; top: -6px; right: -6px; background: #ef4444; color: #fff; border: none; border-radius: 50%; width: 20px; height: 20px; font-size: 0.6rem; cursor: pointer; }
    ...
    <div class="admin-container">
        ...
        <main class="admin-main">
            ...
            <?php if (isset($_GET['success']) || $success): ?>
                <div class="success-message">✅ تم تحديث المنتج بنجاح!</div>
            <?php endif; ?>
                ...
                <form method="POST" enctype="multipart/form-data">
                    <div class="form-row">
                        <div class="form-group">
                            ...
                            <input type="text" id="name" name="name" required placeholder="مثال: قاطع تيار 50 أمبير" value="<?php echo htmlspecialchars($product['name']); ?>">
                        </div>
                        <div class="form-group">
                    ...
                    <div class="form-row">
                        <div class="form-group">
                            ...
                            <input type="text" id="brand" name="brand" placeholder="مثال: شنايدر" value="<?php echo htmlspecialchars($product['brand']); ?>">
                        </div>
                        <div class="form-group">
                            <label for="price">السعر (ج.م) *</label>
                            <input type="number" id="price" name="price" step="0.01" min="0" required placeholder="0" value="<?php echo $product['price']; ?>">
                        </div>
                    </div>
                    ...
                    <div class="form-row">
                        <div class="form-group">
                            ...
                            <input type="text" id="sku" name="sku" placeholder="مثال: SKU-001" value="<?php echo htmlspecialchars($product['sku']); ?>">
                        </div>
                        <div class="form-group">
                            <label for="availability">حالة التوفر</label>
                            <select id="availability" name="availability">
                                <option value="in-stock" <?php echo $product['availability'] == 'in-stock' ? 'selected' : ''; ?>>متوفر</option>
                                <option value="on-demand" <?php echo $product['availability'] == 'on-demand' ? 'selected' : ''; ?>>حسب الطلب</option>
                                <option value="out-of-stock" <?php echo $product['availability'] == 'out-of-stock' ? 'selected' : ''; ?>>غير متوفر</option>
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <textarea id="description" name="description" rows="3" placeholder="وصف المنتج"><?php echo htmlspecialchars($product['description']); ?></textarea>
                    </div>

                    <div class="form-group">
                        <label for="specs">المواصفات التقنية (كل مواصفة في سطر)</label>
                        <textarea id="specs" name="specs" rows="4" placeholder="الجهد: 220 فولت"><?php 
                            $specs = json_decode($product['specs'], true);
                            echo is_array($specs) ? implode("\n", array_map(function($k, $v) { return $k . ': ' . $v; }, array_keys($specs), $specs)) : '';
                        ?></textarea>
                    ...
                    <div class="form-group">
                        ...
                        <textarea id="features" name="features" rows="3" placeholder="حماية قوية"><?php 
                        ...
                        <div class="image-preview">
                            <?php foreach ($images as $img): ?>
                                <div class="image-item">
                                    ...
                                    <a href="edit-product.php?id=<?php echo $product_id; ?>&delete_image=<?php echo $img['id']; ?>" class="delete-image" onclick="return confirm('هل أنت متأكد من حذف هذه الصورة؟')">×</a>
                                </div>
                            <?php endforeach; ?>
                        </div>
                    </div>
